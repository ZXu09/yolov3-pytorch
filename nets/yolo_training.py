...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes, input_shape, cuda, anchors_mask=[[6, 7, 8], [3, 4, 5], [0, 1, 2]]):
        ...
        # -----------------------------------------------------------#
        #   13x13的特征层对应的anchor是[116,90],[156,198],[373,326]小特征层（深层）的感受野大，检测大目标
        ...
        # IOU：预测框和真实框之间的交集和并集之比，缺点：
        # 1.在A框与B框不重合的时候IOU为0，不能正确反映两者的距离大小。
        # 2.无法精确的反映两者的重合度大小。
        # GIOU：囊括预测框和真实框的框 -> C, GIOU = C-二者并集/C
        self.giou = True
    ...
    def get_target(self, l, targets, anchors, in_h, in_w):
        ...
        # -----------------------------------------------------#
        #   用于选取哪些先验框不包含物体
        #   self.anchors_mask[l] = 3
        #   尺寸[batch_size, 3, 13, 13]
        # -----------------------------------------------------#
        ...
        for b in range(bs):
            ...
            #   best_ns:
            #   [每个真实框最大的重合度max_iou - t, 每一个真实框最重合的先验框的序号 - best_n]
            # -------------------------------------------------------#
