...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            # 13×13对应anchors_mask[6,7,8]，对应大尺寸，(深层特征层感受野较大)也就是116,90,  156,198,  373,326
            # 116,90的检测框缩放一下[(416/32,416/32->(13,13), (116/32,90/32)->(3.625,2.8125)]
            ...
            # python切片操作中，...用于代替多个维度，等价于[:, :, 0]，表示最后一个维度的第0列
            # x是一个(batch_size,3,13,13)的数组
            ...
            grid_y = torch.linspace(0, input_height - 1, input_height).repeat(input_width, 1).t(
            ).repeat(batch_size * len(self.anchors_mask[i]), 1, 1).view(y.shape).type(FloatTensor)
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
        ...
        for i, image_pred in enumerate(prediction):
            ...
            conf_mask = (image_pred[:, 4] *
                         class_conf[:, 0] >= conf_thres).squeeze()
