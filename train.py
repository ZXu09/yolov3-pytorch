...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------#
    #   anchors_path    代表先验框对应的txt文件，一般不修改。373,326对应的计算373×326
    #   先验框是在训练集中聚类所得，在yolov3 中每个格子有9个anchor（每个特征层3个）
    #   anchors_mask    用于帮助代码找到对应的先验框，一般不修改。
    #   mask52= [0,1,2] mask26= [3,4,5] mask13= [6,7,8]
    #---------------------------------------------------------------------#
    ...
    if not pretrained:# 没有预训练权重
        weights_init(model)# 初始化权重
    if model_path != '':
        #------------------------------------------------------#
        #   权值文件请看README，百度网盘下载
        #------------------------------------------------------#
        if local_rank == 0:# 进程编号为0，权值文件路径
            print('Load weights {}.'.format(model_path))
        
        #------------------------------------------------------#
        #   根据预训练权重的Key和模型的Key进行加载
        #   model.state_dict() 能够获取模型中的所有参数，包括可学习参数和不可学习参数，其返回值是一个有序字典 OrderedDict。
        #   model中所有的可学习参数（weight、bias）
        #------------------------------------------------------#
        model_dict      = model.state_dict()# 加载模型的字典
        pretrained_dict = torch.load(model_path, map_location = device)# 加载预训练的字典（被修改者）
        # ()表示元组，[]表示列表， {} 字典
        # 列表不能作为字典的key值,而元组可以。字典的key是唯一的
        # 元组通常是由不同的数据组成，而列表是相同的数据队列，元组表示的是结构，而列表表示的是顺序。
        load_key, no_load_key, temp_dict = [], [], {}
        for k, v in pretrained_dict.items():# items()以列表返回可遍历的(键, 值)元组数组
            # pretrained_dict只保留属于model_dict的键，且结构层严格对应相等（维度）
        ...
        model.load_state_dict(model_dict)  # 用一个模型的参数初始化另一个模型的层的操作
        #------------------------------------------------------#
        #   显示没有匹配上的Key
        #------------------------------------------------------#
        if local_rank == 0:
